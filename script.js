document.addEventListener('DOMContentLoaded', () => {
  const url = 'https://script.google.com/a/macros/ext.leroymerlin.com.br/s/AKfycbzSzvlpkcOH1o9a49RCDE7LjsArhrdrlXzOTfd_NATtoiqvEx4oQ4tY4EB6eQB98tLXyw/exec'; // Cole o URL aqui

  // Função para obter os dados dos EPIs
  async function getEpis() {
    const response = await fetch(url + '?action=getEpisData');
    const episData = await response.json();
    renderizarEpis(episData);
  }

  // Função para renderizar a tabela de EPIs
  function renderizarEpis(episData) {
    const tbody = document.querySelector('#tabelaEpis tbody');
    if (!tbody) return;
    tbody.innerHTML = ''; // Limpa a tabela

    episData.forEach(e => {
      const dataFormatada = new Date(e[0]).toLocaleString(); // Formata a data
      tbody.innerHTML += `
        <tr>
          <td>${e[1]}</td>
          <td>${e[2]}</td>
  ...
  // Função para adicionar novo EPI
  async function adicionarEpi(nome, quantidade, categoria, tamanho) {
    await fetch(url + '?action=addEpi&nome=' + nome + '&quantidade=' + quantidade + '&categoria=' + categoria + '&tamanho=' + tamanho);
    getEpis(); // Atualiza a lista de EPIs
  }

  // Exemplo de como adicionar EPI (pode ser um form ou botão)
  document.querySelector('#formAdicionar').addEventListener('submit', (e) => {
    e.preventDefault();
    const form = e.target;
